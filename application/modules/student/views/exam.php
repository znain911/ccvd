...
<html class="no-js" lang="">
    ...
    <body>
        ...
        <div class="exam-container">
			...
			<?php 
				$total_items = $limit;
				$current_item = $this->Exam_model->get_current_item($phase_level, $exm_id);
			?>
			<?php if($current_item != $total_items): ?>
				...
				<div class="container">
					<div class="row">
						...
						<div class="col-lg-6 text-right">
							<div class="timing-count-down" id="timeRemaining" style="display: none;"></div>
							<div class="timing-count-down" >Exam Time: 4.00PM - 5.00PM </div>
						</div>
						<div class="col-lg-12 text-right"><div class="item-no">For students who enter late into the exam, the course administration is not liable for late entry, the exam will finish at 05:00 pm</div></div>
					</div>
				</div>
			</div>
			<?php endif; ?>
			...
			<div id="refreshArea">
				<?php if($current_item == $total_items): ?>
				<?php require_once APPPATH.'modules/student/views/result_of_pca.php'; ?>
				<?php else: ?>
				<?php require_once APPPATH.'modules/student/views/question.php'; ?>
				<?php endif; ?>
			</div>
			<?php echo form_close(); ?>
		</div>
		...
		<script type="text/javascript">
			$("#examEndModule").validate({
				submitHandler : function () {
					...
					$.ajax({
						type : "POST",
						url : baseUrl + "student/exam/answersubmit",
						...
						success : function (data) {
							if(data.status == "ok")
							{
								...
								$('#itemOfTotal').html(data.items_of);
								$('#loader').hide();
								$('[name="_jwar_t_kn_"]').val(data._jwar_t_kn_);
								sqtoken_hash=data._jwar_t_kn_;
								return false;
