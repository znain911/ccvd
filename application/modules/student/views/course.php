			...
			<div class="row">
				...
				<div class="col-lg-9">
					...
					<div class="module-wizard-container">
						<div class="row">
							...
							<div class="<?php echo (count($get_modules) > 3)? 'col-lg-3' : 'col-lg-4' ?>">
								<div style="cursor:pointer;" onclick="window.location.href='<?php echo base_url('student/course?rdr=Course&actmdl='.$module['module_id'].'&shw=true&phase=PCA-1&return=true'); ?>'" class="module-number-stp <?php echo $progress_class; ?>"><?php echo $module['module_name']; ?></div>
							</div>
							<?php 
								$module_x++;
								endforeach; 
					...
					<?php $stdata = $this->Course_model->studentinfodata();
						$exambtch = $this->Course_model->exambatch($stdata->batch_name, 1);
						if (!empty($exambtch)) {
						if ($stdata->student_examstatus == 1) {
							?>
