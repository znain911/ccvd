<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">Pending Students : <span id="totalPendingStudents"><?php echo count($get_items) ?></span></h1>
			...
			<a href="#" class="btn btn-danger pull-right" id="exportpdf"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Export DPF</a>
					...
					<div class="widget p-0">
						<div class="table-responsive">
							...
							<div class="filtar-bar">
								<div class="limit-box">
									<p>
										<span>Search By Batch Name  : &nbsp;&nbsp;</span>
								...
								<table class="table table-td-valign-middle m-b-0">
									...
									<tbody>
										...
										if(count($get_items) !== 0):
											...
											$photo = attachment_url('students/'.$item['spinfo_photo']);
										...
										<tr class="items-row-<?php echo $item['student_id']; ?>">
											...
											<td class="text-center item-row-status-<?php echo $item['student_id']; ?>">
												...
												<span class="btn btn-info btn-xs btn-rounded p-l-10 p-r-10">Waiting</span>
												<?php else: ?>
											...
											<td class="text-center">
												...
												<span class="row-action-status-holder-<?php echo $item['student_id']; ?>">
													...
													<button data-status="2" data-student="<?php echo $item['student_id']; ?>" class="row-action-status btn btn-default btn-xs p-l-10 p-r-10"><i class="fa fa-cog"></i> Waiting</button>
													<button data-status="4" data-student="<?php echo $item['student_id']; ?>" class="row-action-status btn btn-default btn-xs p-l-10 p-r-10"><i class="fa fa-cog"></i> CCVD Sort List</button>
													<!-- <button data-status="5" data-student="<?php echo $item['student_id']; ?>" class="row-action-status btn btn-default btn-xs p-l-10 p-r-10"><i class="fa fa-cog"></i> Reject</button> -->
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(document).on('click', '.row-action-status', function(){
					...
					var status_waiting = '<span class="btn btn-info btn-xs btn-rounded p-l-10 p-r-10">Waiting</span>';
					var status_sort_list = '<span class="btn btn-info btn-xs btn-rounded p-l-10 p-r-10">CCVD Sort List</span>';
					...
					if(staus == '1')
					{
						...
						$('.item-row-status-'+student_id).html(status_waiting);
						$('.row-action-status-holder-'+student_id).html(staus_action_declined);
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/students/waiting',
							data:{student_id:student_id, _jwar_t_kn_:sqtoken_hash},
							dataType: 'json',
							success: function(data)
							{
					...
					{
						$('.item-row-status-'+student_id).html(status_sort_list);
						$('.row-action-status-holder-'+student_id).html(staus_action_approve);
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/students/sort_list',
							data:{student_id:student_id, _jwar_t_kn_:sqtoken_hash},
							dataType: 'json',
							success: function(data)
							{
				...
				$(document).on('click', '#exportpdf', function(){
					var bid = $('#batch').val();
					window.location.href = baseUrl+'coordinator/students/create_pdf/'+bid;					
				});
			});
...
function searchFilter(page_num) {
				...
				$.ajax({
					...
					data:'page='+page_num+'&keywords='+keywords+'&sortby='+sortby+'&limit='+limit+'&from_date='+fromDate+'&to_date='+toDate+'&month='+month+'&batch='+batch+'&year='+year+'&_jwar_t_kn_='+sqtoken_hash,
					dataType:'json',
					beforeSend: function () {
