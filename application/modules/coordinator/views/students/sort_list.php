<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">Students Sort List</a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">Student Sort List : <span id="totalEnrolledStudents"><?php echo count($get_items) ?></span></h1>
			<!-- <a href="<?php echo base_url('coordinator/students/ccvdsortlist_csv');?>" class="btn btn-info pull-right"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export</a> -->
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(document).on('click', '.exportbybatch', function(){
					var bid = $('#batch').val();
					window.location.href = baseUrl+'coordinator/students/ccvdsortlist_csvbybatch/'+bid;					
				});
				$(document).on('click', '#exportpdf', function(){
					var bid = $('#batch').val();
					window.location.href = baseUrl+'coordinator/students/ccvdsortlist_pdf/'+bid;					
				});
			});
			...
			function searchFilter(page_num) {
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url(); ?>studentfilter/get_sort_student/'+page_num,
