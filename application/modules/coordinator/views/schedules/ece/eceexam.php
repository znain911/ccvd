<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">ECE Exam Schedules</a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">ECE Exam Schedules</h1>
			<!-- end page-header -->
			...
			<div class="row">
				...
				<div class="col-md-12 mostak-mdl-box">
					...
					<div id="moduleLessonDisplay">
						<?php require_once APPPATH.'modules/coordinator/views/schedules/ece/contents.php'; ?>
					</div>
				</div>
				<!-- end col-4 -->
			</div>
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.remove-row', function(){
					if(confirm('Are you sure?', true))
					{
						...
						var ece_id = $(this).attr('data-phase');
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/schedules/ece_delschedule',
							data:{schedule_id:schedule_id, ece_id:ece_id, _jwar_t_kn_:sqtoken_hash},
							dataType: 'json',
							beforeSend: function()
							{
								$('#mdlBoxLoader').show();
								...
								if(data.status == 'ok')
								{
									...
									$("#createData").validate({
										...
										submitHandler : function () {
											...
											$.ajax({
												type : "POST",
												url : baseUrl + "coordinator/schedules/ece_create",
				...
				$(document).on('click', '.row-action-edit', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/schedules/ece_edit",
							...
							if(data.status == 'ok')
							{
								...
								$("#updateData").validate({
									...
									submitHandler : function () {
										...
										$.ajax({
											type : "POST",
											url : baseUrl + "coordinator/schedules/ece_update",
				...
				$(document).on('click', '.retrive-create-again', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/schedules/ece_schedule",
						data:{_jwar_t_kn_:sqtoken_hash, ece_id:ece_id},
						dataType: 'json',
						beforeSend: function()
						{
							$('#mdlBoxLoader').show();
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(document).on('click', '.row-action-center', function(){
					...
					$.ajax({
						type: 'POST',
						url: baseUrl + 'coordinator/schedules/ece_lesson',
						data:{schedule_id:schedule_id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						beforeSend: function(){
							$('#mdlBoxLoader').show();
							...
							if(data.status == 'ok')
							{
								...
								$("#createCenterData").validate({
									...
									submitHandler : function () {
										...
										$.ajax({
											type : "POST",
											url : baseUrl + "coordinator/schedules/ece_createlesson",
				...
				$(document).on('click', '.return-create-lesson', function(){
					...
					$.ajax({
						type : "POST",
						url : baseUrl + "coordinator/schedules/ece_getlessoncreate",
						data : {schedule_id:schedule_id, _jwar_t_kn_:sqtoken_hash},
						dataType : "json",
						beforeSend: function(){
							$('#mdlBoxLoader').show();
				...
				$(document).on('click', '.row-action-edit-lesson', function(){
					...
					$.ajax({
						type : "POST",
						url : baseUrl + "coordinator/schedules/ece_editlesson",
						data : {schedule_id:schedule_id, _jwar_t_kn_:sqtoken_hash},
						dataType : "json",
						beforeSend: function(){
							$('#mdlBoxLoader').show();
							...
							if(data.status == "ok")
							{
								...
								$("#updateCenterData").validate({
									...
									submitHandler : function () {
										...
										$.ajax({
											type : "POST",
											url : baseUrl + "coordinator/schedules/ece_updatelesson",
				...
				$(document).on('click', '.remove-row-lesson', function(){
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/schedules/ece_dellesson',
				...
				$(document).on('click', '.back-to-modules', function(){
					...
					$.ajax({
					   type:"POST",
					   url: baseUrl + "coordinator/schedules/ece_backtoschedules",
					   data: {ece_id:ece_id, _jwar_t_kn_:sqtoken_hash},
					   dataType: 'json',
					   cache: false,
					   beforeSend: function(){
						 $('#mdlBoxLoader').show();  
