					...
					<div class="content-text-goes">						
						...
						<table class="table">
						  ...
						  <tbody>
						  	...
						  	<?php $sl = 1; foreach ($ece_result as $examlst) {?>
						<?php $total_right_answers = $this->Results_model->get_right_answers_ece($examlst->eceexmcount_student_id);
                        $total_wrong_answers = $this->Results_model-> get_wrong_answers_ece($examlst->eceexmcount_student_id);

                       
                         /*print_r($examlst->eceexmcount_id);*/
						/*print_r($total_wrong_answers);*/
						if(count($total_right_answers) != 0 && count($total_wrong_answers) != 0){
		  	               ?>
						  		<tr>
							      <th scope="row"><?php echo $sl;?></th>
							      <td><?php echo $examlst->eceexmcount_date?></td>
								  ...
								  <td>
                                      <a href="javascript:void(0);" class="btn btn-info btn-xs p-l-10 p-r-10" data-toggle="modal" data-target="#myModal" onclick="data_view('<?php echo $examlst->eceexmcount_student_id; ?>')">View</a>
                                   </td>
							    </tr>
						<?php $sl++; } }?>
		...
		<script type="text/javascript">
			function data_view(id){ 
			 ...
			 $('#qsndata').html(loader);         
			    ...
			    $.ajax({
			        url:"<?php echo base_url(); ?>/coordinator/results/viewdetails_ece",  
			        method:"POST",  
			        data:{id:id},  
