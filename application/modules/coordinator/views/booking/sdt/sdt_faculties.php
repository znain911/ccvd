<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		<!-- end #header -->
		<?php $get_applicants = $this->Booking_model->get_sdt_applicants('Faculty'); ?>
		<!-- begin #sidebar -->
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">SDT Applicants (Faculties)</a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">SDT Applicants (Faculties) : <span id="totalSdtBooking"><?php echo count($get_applicants) ?></span></h1>
			<!-- end page-header -->
								...
								<table id="data-table" class="table table-td-valign-middle m-b-0">
									...
									<tbody class="appnd-not-fnd-row">
										<?php 
											...
											$booking_status = $this->Booking_model->check_sdtbooking_status($applicant['booking_id'], $applicant['booking_user_id']);
										?>
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.remove-row', function(){
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/booking/sdt_delete',
				...
				$(document).on('click', '.accept-row', function(){
					...
					$.ajax({
						type: 'POST',
						url: baseUrl+'coordinator/booking/as_selected',
				...
				$(document).on('click', '.decline-row', function(){
					...
					$.ajax({
						type: 'POST',
						url: baseUrl+'coordinator/booking/as_declined',
						data:{booking_id:booking_id, faculty_id:faculty_id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						success: function(data)
						{
